...
| plan_start_at | integer | 计划开始时间(Unix 毫秒),可空 |
| plan_due_at | integer | 预计完成时间(Unix 毫秒),可空;超期判定依据 |
| completed_at | integer | 实际完成时间(进入 done 时写入),可空 |
| created_at / updated_at | integer NOT NULL | |
...
| | `apply_analysis_run` | runId, selectedRequirementIds?, conflictDecisions? | 选中需求事务落库为 draft(默认全选);相悖块必须携带裁决结果(用新/用旧/都保留),重复块携带处置(并入/仍要新建/跳过) |
