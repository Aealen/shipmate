...
11. **Deadline 识别与继承(工作台素材分析)**:需求块与需求点均支持 Deadline(YYYY-MM-DD),默认由 AI 从素材时间性表述识别(「一周内/X月X日前」等,以素材日期为基准换算;今天日期注入 system prompt);纯时间性/排期性表述识别为 deadline,不拆成需求点。约束:点的 deadline 包含在块的 deadline 之内(超块拒绝保存),点未单独设置时默认继承块 deadline(块 deadline 变更时,未单独设置/等于旧块值的点跟随)。均可通过编辑弹窗修改(date input)。落库:应用时块 deadline → requirements.plan_due_at,点 deadline → requirement_points.plan_due_at(未单独给出则继承块值);llm/schema 的 draftRequirement/draftPoint 增加 deadline 可空字段
