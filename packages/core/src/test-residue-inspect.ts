...
async function main(): Promise<void> {
  ...
  for (const r of logs) {
    console.log(r.entityType, r.entityId.slice(0, 24), r.changeType, (r.reason ?? '').slice(0, 40), new Date(r.createdAt).toISOString());
  }
  const counts = [
  ...
  for (const [name, t] of counts) {
    ...
    for (const r of rows) console.log('   ', (r as { id: string }).id.slice(0, 24), String((r as { title?: string; name?: string }).title ?? (r as { name?: string }).name ?? '').slice(0, 30));
  }
  process.exit(0);
}
